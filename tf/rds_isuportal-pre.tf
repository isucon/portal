resource "aws_rds_cluster" "isuxportal-pre" {
  cluster_identifier              = "isuxportal-pre"
  master_username                 = "root"
  master_password                 = random_password.isuxportal-rds-pre.result
...
resource "random_password" "isuxportal-rds-pre" {
  length           = 16
  special          = true
  override_special = "_%@"
}

resource "aws_rds_cluster_instance" "isuxportal-pre-001" {
  identifier                   = "isuxportal-pre-001"
  cluster_identifier           = aws_rds_cluster.isuxportal-pre.cluster_identifier
