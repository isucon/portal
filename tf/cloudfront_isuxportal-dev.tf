resource "aws_cloudfront_distribution" "isuxportal-dev" {
  enabled         = true
  is_ipv6_enabled = true
  comment         = "isuxportal-dev"
  aliases         = ["portal-dev.isucon.net"]
  ...
  origin {
    origin_id   = "portal-dev"
    domain_name = "portal-dev.xi.isucon.dev"
  ...
  default_cache_behavior {
    ...
    forwarded_values {
      ...
      cookies {
        forward           = "whitelist"
        whitelisted_names = ["isuxportaldev_sess"]
      }
    }
    min_ttl                = 0
    default_ttl            = 0
