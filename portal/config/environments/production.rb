Rails.application.configure do
  ...
  config.x.contest.max_teams = Integer(ENV.fetch('ISUXPORTAL_MAX_TEAMS'))
  ...
  config.x.checker_token.secret = ENV.fetch('ISUXPORTAL_CHECKER_SECRET').unpack1('m0') # ruby -rsecurerandom -e 'puts SecureRandom.base64(96)'
  ...
  config.x.ami.test_id = ENV.fetch('ISUXPORTAL_TEST_AMI_ID')
  config.x.ami.qualify_id = ENV['ISUXPORTAL_QUALIFY_AMI_ID']
end
