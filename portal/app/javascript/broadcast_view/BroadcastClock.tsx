...
export const BroadcastClock: React.FC<Props> = ({contest}) => {
  ...
  const contestFreezesAt = dayjs((contest.freezesAt!.seconds as number) * 1000 + (contest.freezesAt!.nanos as number) / 1000000);
  ...
  let status = "running";
  let text = <p>{digits(remaining.hours())}:{digits(remaining.minutes())}:{digits(remaining.seconds())}</p>
  let progress = <progress className="progress is-small is-link" max={100} value={100-((untilEnd/duration)*100)}></progress>;
  if (untilEnd <= 0) {
    status = "end";
    text = <p>00:00:00</p>;
    ...
    status = "frozen";
    progress = <progress className="progress is-small is-danger" max={100} value={100-((untilEnd/duration)*100)}></progress>;
  ...
  return <div className={`isux-broadcast-clock isux-broadcast-clock--${status} mt-1`}>
    <div className="columns">
      <div className="column is-12 isux-broadcast-clock-text px-0 mx-0 py-0 my-0">
        {text}
      </div>
    </div>
    <div className="columns">
