...
export class AdminTeamDetail extends React.Component<Props, State> {
    ...
    return <>
      ...
      <div className="card mt-5">
        ...
        <div className="card-content">
          ...
          <p>代表者メールアドレス: <a href={`mailto:${this.state.team.detail!.emailAddress}`}>{this.state.team.detail!.emailAddress}</a></p>

          <p><Link to={`/admin/teams/${this.state.team.id}/edit`} className="button is-info">編集</Link></p>
        </div>
      </div>
    </>;
  }
  ...
  renderMember(member: isuxportal.proto.resources.IContestant) {
    return <div className="card mt-4" key={member.id as number}>
        <div className="card-content">
          <div className="media">
            <div className="media-left">
              <figure className="image is-48x48">
                <img src={member.contestantDetail!.avatarUrl || "https://avatars2.githubusercontent.com/u/10137?s=144"} />
              </figure>
            </div>
            <div className="media-content">
              <p className="title is-5">{member.name}</p>
                <p className="subtitle is-6">
                  ...
                  <span>
                    GitHub:
                    <a href={`https://github.com/${member.contestantDetail!.githubLogin}`}>@{member.contestantDetail!.githubLogin}</a>
                  </span>
                  <span>
                    Discord:
  ...
  teamIsStudent() {
    return this.state.team?.members && this.state.team.members.filter((v) => !v.contestantDetail!.isStudent).length == 0
  }
